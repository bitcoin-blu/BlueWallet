...
export class HDLegacyBreadwalletWallet extends HDLegacyP2PKHWallet {
  ...
  _calcNodeAddressByIndex(node: number, index: number, p2wpkh: boolean = false) {
    ...
    const address = p2wpkh ? bitcoinjs.payments.p2wpkh({ pubkey, network: bbluNetwork }).address : bitcoinjs.payments.p2pkh({ pubkey, network: bbluNetwork }).address;
  ...
  _addPsbtInput(psbt: Psbt, input: CoinSelectReturnInput, sequence: number, masterFingerprintBuffer: Uint8Array) {
    ...
    const ProxyClass = input?.address?.startsWith(BBLU_BECH32_PREFIX) ? AbstractHDElectrumWallet : HDLegacyP2PKHWallet;
